...
export async function getGalleryCategories() {
  ...
  try {
    ...
    return db.select().from(galleryCategories).orderBy(galleryCategories.displayOrder);
  } catch (error) {
...
export async function getGalleryCategoryBySlug(slug: string) {
  ...
  try {
    const { galleryCategories } = await import("../drizzle/schema");
    const result = await db.select().from(galleryCategories).where(eq(galleryCategories.slug, slug));
    return result[0] || null;
  } catch (error) {
    console.error("[Gallery] Error getting category by slug:", error);
    throw error;
  }
}
...
export async function getGalleryImagesByCategory(categoryId: number) {
  ...
  try {
    ...
    console.error("[Gallery] Error getting images by category:", error);
    throw error;
  }
}
...
export async function deleteGalleryCategory(id: number) {
  ...
  try {
    ...
    await db.delete(galleryImages).where(eq(galleryImages.categoryId, id));
    return db.delete(galleryCategories).where(eq(galleryCategories.id, id));
  } catch (error) {
    console.error("[Gallery] Error deleting category:", error);
    throw error;
  }
}
