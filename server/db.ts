...
import { InsertUser, users, events, InsertEvent, Event, contacts, InsertContact, Contact, newsletterSubscribers, InsertNewsletterSubscriber, NewsletterSubscriber, newsletterCampaigns, InsertNewsletterCampaign, NewsletterCampaign, unsubscribeTokens, InsertUnsubscribeToken, UnsubscribeToken, newsletterOpens, newsletterClicks, blogPosts, BlogPost, InsertBlogPost, administrators, Administrator, InsertAdministrator, facebookShares, FacebookShare, InsertFacebookShare, facebookEngagementMetrics, FacebookEngagementMetrics, InsertFacebookEngagementMetrics, facebookWebhookEvents, FacebookWebhookEvent, InsertFacebookWebhookEvent } from "../drizzle/schema";
...
export async function createEngagementMetrics(data: InsertFacebookEngagementMetrics): Promise<FacebookEngagementMetrics> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  try {
    const result = await db.insert(facebookEngagementMetrics).values(data);
    const metrics = await db
      .select()
      .from(facebookEngagementMetrics)
      .where(eq(facebookEngagementMetrics.id, result[0].insertId));
    return metrics[0];
  } catch (error) {
    console.error("[Facebook] Error creating engagement metrics:", error);
    throw error;
  }
}
...
export async function updateEngagementMetrics(
  ...
  data: Partial<InsertFacebookEngagementMetrics>
): Promise<FacebookEngagementMetrics | null> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  try {
    await db
      .update(facebookEngagementMetrics)
      .set(data)
      .where(eq(facebookEngagementMetrics.facebookPostId, facebookPostId));
    ...
    console.error("[Facebook] Error updating engagement metrics:", error);
    throw error;
  }
}

export async function getEngagementMetrics(facebookPostId: string): Promise<FacebookEngagementMetrics | null> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getEngagementMetricsByShareId(facebookShareId: number): Promise<FacebookEngagementMetrics | null> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  try {
    const metrics = await db
      .select()
      .from(facebookEngagementMetrics)
      .where(eq(facebookEngagementMetrics.facebookShareId, facebookShareId));

    return metrics[0] || null;
  } catch (error) {
    console.error("[Facebook] Error getting engagement metrics by share ID:", error);
    throw error;
  }
}

export async function getAllEngagementMetrics(): Promise<FacebookEngagementMetrics[]> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  try {
    const metrics = await db
      .select()
      .from(facebookEngagementMetrics)
      .orderBy(desc(facebookEngagementMetrics.lastUpdated));
...
export async function logWebhookEvent(data: InsertFacebookWebhookEvent): Promise<FacebookWebhookEvent> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  try {
    const result = await db.insert(facebookWebhookEvents).values(data);
    const event = await db
      .select()
      .from(facebookWebhookEvents)
      .where(eq(facebookWebhookEvents.id, result[0].insertId));

    return event[0];
  } catch (error) {
    console.error("[Facebook] Error logging webhook event:", error);
    throw error;
  }
}
...
export async function updateWebhookEventStatus(
  ...
  processed: boolean,
  errorMessage?: string
): Promise<FacebookWebhookEvent | null> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  try {
    await db
      .update(facebookWebhookEvents)
      .set({
        processed: processed ? 1 : 0,
        errorMessage: errorMessage || null,
      })
      .where(eq(facebookWebhookEvents.id, eventId));
    ...
    console.error("[Facebook] Error updating webhook event status:", error);
    throw error;
  }
}

export async function getUnprocessedWebhookEvents(): Promise<FacebookWebhookEvent[]> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  try {
    const events = await db
      .select()
      .from(facebookWebhookEvents)
      .where(eq(facebookWebhookEvents.processed, 0))
      .orderBy(facebookWebhookEvents.createdAt);

    return events;
  } catch (error) {
    console.error("[Facebook] Error getting unprocessed webhook events:", error);
    throw error;
  }
}

export async function getWebhookEventsByPostId(facebookPostId: string): Promise<FacebookWebhookEvent[]> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  try {
    const events = await db
      .select()
      .from(facebookWebhookEvents)
      .where(eq(facebookWebhookEvents.facebookPostId, facebookPostId))
      .orderBy(desc(facebookWebhookEvents.createdAt));

    return events;
  } catch (error) {
    console.error("[Facebook] Error getting webhook events by post ID:", error);
    throw error;
  }
}
